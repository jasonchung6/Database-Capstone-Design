...
CREATE TABLE Room (
  R_Id VARCHAR(45) NOT NULL,        -- 강의실 ID PRIMARY KEY로 설정
  R_Name VARCHAR(45) NOT NULL,      -- 강의실의 이름
  Capacity VARCHAR(45) NULL,        -- 강의실의 수용 인원
  B_Id INT NOT NULL,                -- 건물의 ID (외래 키) PRIMARY KEY로 설정
  D_Id VARCHAR(5) NOT NULL,         -- 학과의 아이디 (외래 키)
...
CREATE TABLE Department (
  ...
  D_Name VARCHAR(45) NOT NULL,      -- 학과명
...
-- Dep_Stu 테이블 생성: 학과와 학생의 관계를 저장 (다대다 관계 테이블)
CREATE TABLE Dep_Stu (
  D_Id_Major VARCHAR(5) NOT NULL,   -- 학과 ID (외래 키)
  S_Id INT NOT NULL,                -- 학생 ID (외래 키)
  PRIMARY KEY (D_Id_Major, S_Id),
  FOREIGN KEY (D_Id_Major) REFERENCES Department (D_Id),-- 학과 ID에 대한 외래 키 제약 조건
  FOREIGN KEY (S_Id) REFERENCES Student (S_Id)  -- 학생 ID에 대한 외래 키 제약 조건
);
